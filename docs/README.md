# NEXUS PRIME — Documentation

**Start here:** [00_START_HERE_MASTER_GUIDE.md](00_START_HERE_MASTER_GUIDE.md)

---

## Current Documentation (New)

| Document | Purpose |
|----------|---------|
| [00_START_HERE_MASTER_GUIDE.md](00_START_HERE_MASTER_GUIDE.md) | Entry point, deployment 101 |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Technical architecture |
| [API_REFERENCE.md](API_REFERENCE.md) | API reference (Cortex, Auth, LiteLLM) |
| [DEPLOYMENT.md](DEPLOYMENT.md) | Deployment guide (AR) |
| [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md) | Deployment guide (EN) |
| [LOCAL_DEPLOYMENT.md](LOCAL_DEPLOYMENT.md) | Local deployment |
| [NEXUS_PRIME_TECHNICAL_DOCUMENTATION_AR.md](NEXUS_PRIME_TECHNICAL_DOCUMENTATION_AR.md) | Technical docs (AR) |
| [README_AR.md](README_AR.md) | Overview (AR) |
| [QUICKSTART.md](QUICKSTART.md) | Quick start |
| [ECOSYSTEM_API_README.md](ECOSYSTEM_API_README.md) | Ecosystem API |
| [SOVEREIGN_ARCHITECTURAL_GOVERNANCE.md](SOVEREIGN_ARCHITECTURAL_GOVERNANCE.md) | Sovereign governance |
| [SOVEREIGN_ENCYCLOPEDIA.md](SOVEREIGN_ENCYCLOPEDIA.md) | Sovereign encyclopedia |
| [COMPREHENSIVE_ASSESSMENT_2026.md](COMPREHENSIVE_ASSESSMENT_2026.md) | 2026 assessment |
| [EXECUTIVE_SITE_INSPECTION_REPORT.md](EXECUTIVE_SITE_INSPECTION_REPORT.md) | Site inspection |
| [SECURE_THE_EMPIRE_SSL_AND_LIVE_AUDIT.md](SECURE_THE_EMPIRE_SSL_AND_LIVE_AUDIT.md) | SSL audit |
| [IDENTITY_COMPLIANCE_PROTOCOL_KIER.md](IDENTITY_COMPLIANCE_PROTOCOL_KIER.md) | Compliance protocol |
| [LAUNCH_READINESS_CHECKLIST.md](LAUNCH_READINESS_CHECKLIST.md) | Launch checklist |

---

## Folders

| Folder | Purpose |
|--------|---------|
| [2026-02-24/](2026-02-24/) | تقارير خطة الإصلاح — REMAINING_MANUAL، ORGANIZATION_SNAPSHOT، FULL_SERVER_ANALYSIS، NEXT_PHASE_PLAN |
| [Science_and_RnD/](Science_and_RnD/) | Patents, scientific deconstruction, investment prospectus |
| [archive/](archive/) | Historical reports, imported docs, raw archives |
| [PERSONAL_DOCS/](PERSONAL_DOCS/) | Personal documents (IP, certificates, Shadow7) |

---

*Excluded from cleanup: planets/SHADOW-7, nexus_nerve, neural_spine, sovereign_dify_bridge, nexus_oracle*
