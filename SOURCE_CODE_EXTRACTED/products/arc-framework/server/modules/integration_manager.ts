...
async function callGemini(request: LLMRequest): Promise<string | null> {
  const LITELLM_URL = process.env.LITELLM_PROXY_URL || "http://nexus_litellm:4000";
  const MASTER_KEY = process.env.LITELLM_MASTER_KEY || "sk-nexus-sovereign-mrf103";

  const model = request.model || "gemini-pro";
  const url = `${LITELLM_URL}/v1/chat/completions`;

  // OpenAI-compatible format (LiteLLM handles Gemini routing)
  ...
  if (!res.ok) {
    log(`❌ LiteLLM proxy error: ${res.status}`, "litellm");
    return null;
  }
