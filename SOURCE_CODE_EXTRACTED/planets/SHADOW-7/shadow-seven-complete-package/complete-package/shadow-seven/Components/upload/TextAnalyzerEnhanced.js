/**
 * Text Analyzer Enhanced - محسّن بمعالجة محلية + نظام الوكلاء
 * يستخدم NLP المحلي + الوكلاء المتخصصين لجودة دور النشر
 */
...
export async function quickFileAnalysis(rawContent) {
  ...
  return {
    ...
    language,
    detected_chapters: structure.chapters?.length || 0,
    detected_pages: structure.pages?.length || 0,
    has_toc: (structure.toc?.length || 0) > 0,
    repetition_rate: duplicates.overall?.repetitionRate || 0,
