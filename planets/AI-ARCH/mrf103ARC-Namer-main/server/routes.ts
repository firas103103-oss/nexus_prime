...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Register Legal Sector routes
  const legalRoutes = (await import("./routes/legal")).default;
  app.use("/api/legal", legalRoutes);
  
  // Register Life Management routes
  const lifeRoutes = (await import("./routes/life")).default;
  app.use("/api/life", lifeRoutes);
  
  // Register R&D Lab routes
  const rndRoutes = (await import("./routes/rnd")).default;
  ...
  app.post("/api/auth/login", authLimiter.middleware(), async (req: any, res) => {
    // Direct login - no password verification required
    logger.info('🔓 Direct login attempt from IP:', getClientIp(req));
    ...
    req.session.save(async (err: any) => {
      ...
      console.log('✅ Direct login successful for IP:', getClientIp(req));
      res.json({ ok: true, message: "Authentication successful - Direct access enabled" });
    });
  });
