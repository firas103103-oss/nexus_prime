...
// App mode: 'terminal' | 'modern'
type AppMode = 'terminal' | 'modern';

const App = () => {
  // Check if user prefers modern UI
  const [appMode, setAppMode] = useState<AppMode>(() => {
    const saved = localStorage.getItem('appMode');
    return (saved as AppMode) || 'modern'; // Default to modern
  });
  ...
  const toggleMode = () => {
    setAppMode(prev => prev === 'terminal' ? 'modern' : 'terminal');
  };
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMode}
        className="fixed top-4 right-4 z-50 px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2"
        title={appMode === 'terminal' ? 'Switch to Modern UI' : 'Switch to Terminal UI'}
      >
        <span className="text-sm font-medium">
          {appMode === 'terminal' ? '🎨 Modern' : '💻 Terminal'}
        </span>
      </button>

      {/* Render based on mode */}
      {appMode === 'terminal' ? <TerminalApp /> : <ModernApp />}
    </>
  );
};

// Terminal Mode App (Original App.tsx)
const TerminalApp = () => {
  // Original App.tsx implementation
  // Import all the logic from App.tsx here
  // For now, placeholder:
  return (
    <div className="min-h-screen bg-black text-green-400 p-8">
      <h1 className="text-4xl font-bold mb-4">Terminal Mode</h1>
      <p className="text-xl">Original Terminal UI will be here</p>
      <p className="text-sm opacity-70 mt-4">This is the classic terminal interface</p>
    </div>
  );
};

// Modern Mode App (AppV2.tsx)
const ModernApp = () => {
  ...
  const handleSendMessage = async (content: string, files?: File[]) => {
    ...
    if (files && files.length > 0) {
      ...
      // Regular chat response with API call
      setTimeout(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        const completionMessages = {
          ar: '✅ اكتمل! تم معالجة مخطوطتك بنجاح.\n\nيمكنك الآن تحميل حزمة النشر الكاملة.',
          en: '✅ Complete! Your manuscript has been successfully processed.\n\nYou can now download the complete publishing package.',
          de: '✅ Fertig! Ihr Manuskript wurde erfolgreich verarbeitet.\n\nSie können jetzt das vollständige Veröffentlichungspaket herunterladen.'
        };
