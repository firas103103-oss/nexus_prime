...
function App() {
  const { language, visualMode, accentColor, overlayMessage, report, processInput, selectLanguage, purgeSystem, isProcessing, hiveDialogue, lastHqAudio } = useNexusCore();
  ...
  const handleDownload = () => {
    ...
    // We pass lastHqAudio to embed it directly in the HTML
    const html = generateSmartHTMLReport(report, hiveDialogue, language, lastHqAudio || undefined);
      ...
      {visualMode !== 'INTRO_MODE' && (
        <div className="z-10 w-full h-full flex flex-col justify-between py-12 px-6 pointer-events-none">
          ...
          <div className="w-full max-w-5xl mx-auto flex flex-col items-center gap-6">
            ...
            {!report && (
              <div className={`w-full max-w-3xl pointer-events-auto mt-4 transition-opacity duration-300 ${isProcessing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                 <div className="flex gap-4 items-center bg-black/40 backdrop-blur-md p-2 rounded-full border border-white/10 hover:border-white/20 transition-all hover:shadow-[0_0_20px_rgba(0,242,255,0.1)]">
                    <input type="file" ref={fileInputRef} onChange={(e) => { if(e.target.files?.[0]) processInput("Analyze this.", e.target.files[0]) }} className="hidden" disabled={isProcessing} />
                    <button onClick={() => fileInputRef.current?.click()} className="p-3 rounded-full bg-white/5 hover:bg-white/10 text-gray-400 hover:text-cyan-400 transition" disabled={isProcessing}><FileText size={20} /></button>
                    <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && inputValue) processInput(inputValue) }}
                      ...
                      className="flex-1 bg-transparent border-none text-center text-lg focus:outline-none placeholder-white/20 h-10" style={{ caretColor: accentColor }} autoFocus disabled={isProcessing} />
                 </div>
              </div>
            )}
          </div>
