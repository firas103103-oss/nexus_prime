...
const App = () => {
  ...
  const runProcessingPipeline = async () => {
    ...
    try {
        ...
        log(lang === 'ar' ? "بدء تشغيل المحركات (Gemini 3 Pro)..." : "Initializing Engines (Gemini 3 Pro)...");
        ...
        agentSpeak(lang === 'ar' 
            ? "تمت المهمة بنجاح تام. هذه هي الحزمة النهائية، تتضمن المخطوطة المحررة، التقارير، الغلاف، ورسالة خاصة مني نيابة عن منظمة MrF."
            : "Mission accomplished successfully. Here is your final package including edited manuscript, reports, cover, and a special letter on behalf of MrF Organization.");
      ...
      <header className="h-16 border-b border-slate-900 bg-slate-950/80 backdrop-blur fixed w-full top-0 z-50 flex items-center justify-between px-6 shadow-lg shadow-black/20">
        <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-[10px] text-gold-500/80 tracking-widest">MrF X OS ORGANIZATION</p>
             </div>
        </div>
         ...
         {messages.map((msg) => (
             <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                 <div className={`max-w-[90%] md:max-w-[70%] rounded-2xl p-4 shadow-xl relative overflow-hidden ${
                     ...
                     {msg.options && (
                         <div className="mt-4 flex flex-col gap-3" role="group" aria-label="options">
                             {msg.options.map((opt, idx) => (
                                 <button 
                                    key={idx}
                                    onClick={() => handleOptionSelect(opt.value)}
                                    className="px-5 py-4 bg-slate-800 hover:bg-gold-500 hover:text-black text-left border border-slate-700 hover:border-gold-500 transition-all group rounded-xl touch-target"
                                    role="option"
                                    aria-label={opt.description ? `${opt.label}: ${opt.description}` : opt.label}
                                 >
                                     <div className="font-bold text-sm mb-1 text-slate-200 group-hover:text-black">
                                         {opt.label}
                                     </div>
                                     {opt.description && (
                                         <div className="text-xs text-slate-500 group-hover:text-black/70">
                                             {opt.description}
                                         </div>
                                     )}
                                 </button>
                ...
                {previewHtmlBlob && (
                 ...
                 <button 
                    onClick={() => {
                        ...
                        a.href = url;
                        a.download = `MrF_Package_${metadata.title.replace(/\s/g, '_')}.zip`;
                        a.click();
                    }}
          ...
          <footer className="fixed bottom-0 w-full bg-slate-950 border-t border-slate-900 z-40 backdrop-blur-lg bg-opacity-90">
            ...
            <div className="border-t border-slate-800 py-2 text-center">
              <p className="text-xs text-gold-500 font-semibold flex items-center justify-center gap-2">
                <Terminal size={14} />
                Powered by MrF X OS Organization
              </p>
            </div>
          </footer>
      )}
